...
public class ProgrammeDao extends AbstractDao<Programme, Long> {
    ...
    @Override
    protected final void attachEntity(Programme entity) {
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getChannelGuideDao().getAllColumns());
            builder.append(" FROM PROGRAMME T");
            builder.append(" LEFT JOIN CHANNEL_GUIDE T0 ON T.\"CHANNEL\"=T0.\"ID\"");
            builder.append(' ');
    ...
    protected Programme loadCurrentDeep(Cursor cursor, boolean lock) {
        Programme entity = loadCurrent(cursor, 0, lock);
        int offset = getAllColumns().length;

        ChannelGuide channelGuide = loadCurrentOther(daoSession.getChannelGuideDao(), cursor, offset);
        entity.setChannelGuide(channelGuide);

        return entity;    
    }

    public Programme loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<Programme> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<Programme> list = new ArrayList<Programme>(count);
    ...
    protected List<Programme> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<Programme> queryDeep(String where, String... selectionArg) {
